...
;            $.each($(this).prev('ul').find(".number"), function (i) {
                if ($ul_total.eq(i).attr("checked") == "checked") {
                    ...
                    produceIds = produceIds + "," + $(this).next().html()
                }
            });
            $(this).children("span").html("￥" + s.toFixed(1));
            $(this).children("number").html(n1);
            $(this).children("a").html(produceIds);
            S = S + s;
        });
...
$(function () {
    ...
    $('.sett').click(function () {
        var a = $(".total").children("number").html();
        var b = $(this).prev("span").html();
        var c = $(".total").children("a").html();
        var item = { "produceId": c,"orderNum":a,"totalPrice":b};
        $.ajax({
            type: "POST",
